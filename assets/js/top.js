...
(function () {
  ...
  $_window.on("resize", function () {
    scroll = $_window.scrollTop(), wH = $_window.height(), wW = $_window.width();
  });
  ...
  SectionModuleSuper.prototype = {
    "super": function _super(section, obj) {
    ...
    Parallax: function Parallax(rate) {//this.Obs.trigger("Parallax", { rate: rate });
    },
  ...
  ScrollAnimation.prototype = {
    ...
    getSection: function getSection(index) {
      var i = index - 1;
      return this.sections[i];
    },
    Reset: function Reset(section) {
      if (this.sectionObjects[section]) {
        this.sectionObjects[section].Reset();
      }
    },
    Start: function Start(section) {
      if (this.sectionObjects[section]) {
        this.sectionObjects[section].Start();
      }
    },
    paraStart: function paraStart(section, rate) {
      if (this.sectionObjects[section]) {
    ...
    ScrollEvent: function ScrollEvent() {
      ...
      for (var i = 0; i < len; i++) {
        ...
        if (animateSection.pos < scroll02 && scroll02 < animateSection.pos + animateSection.h) {
          current = sectionName;
        }
      }
